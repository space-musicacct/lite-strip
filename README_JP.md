...
const strip = async (url) => {
  const res = await fetch(`http://localhost:8080/?url=${encodeURIComponent(url)}&timezone=Asia/Tokyo`);
...
def strip(url: str) -> dict:
    res = requests.get("http://localhost:8080/", params={"url": url, "timezone": "Asia/Tokyo"})
    return res.json()
